            ...
            <form method="POST" action="/code/{{ $kodeMaterial->id }}">
                ...
                <div class="row g-3">
                      <div class="col-md-3 mb-3">
                        ...
                        <input class="form-control @error('peruntukan') is-invalid @enderror" type="text" placeholder="Ketikkan kode..." name="peruntukan" id="peruntukan" value="{{ old('peruntukan',$kodeMaterial->peruntukan) }}" onkeyup="this.value = this.value.toUpperCase()" required autofocus>
                      </div>
                      @error('peruntukan')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
